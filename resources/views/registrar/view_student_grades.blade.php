        ...
        <table class="table">
            ...
            <tbody>
                @forelse ($student->grades as $grade)
                    <tr>
                        <td>{{ $grade->subject }}</td>
                        <form action="{{ route('update_grades', ['id' => $grade->id]) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('PUT')
                            <td>
                        ...
                        <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('{{ $grade->id }}', '{{ $grade->subject }}')">Delete</button>
                        </td>
                    </tr>
                @empty
                    <tr>
